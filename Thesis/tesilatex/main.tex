              %*****************************************%
              %                                         %
              % Template standard per la Tesi di Laurea %
              %            di Stefano Bianchi           %
              %                                         %
              %         versione: 27 marzo 2014         %
              %                                         %
              %   Updated and translated into English   %
              %             by Tommaso Fonda            %
              %                                         %
              %         revision: 6th Nov. 2023         %
              %                                         %
              %*****************************************%

% Replace 'oneside' with 'twoside' for double-side printing
\documentclass[a4paper,12pt,oneside,openany]{book}

%*******************************************************
% Packages
%*******************************************************
%\usepackage[a4paper,top=3cm,bottom=3cm,left=3cm,right=3cm]{geometry}
\usepackage{geometry}
% Typographical rules for the English language
\usepackage[english]{babel}
% Provides the \thetitle and other useful macros
\usepackage{titling}

% Needed for the transparent full-page background logo
\usepackage{eso-pic}
\usepackage{transparent}
% Font-related packages
\usepackage[utf8]{inputenc}
\usepackage[T1]{fontenc}
\usepackage{inconsolata}

% Needed to render the cover
\usepackage{fancyhdr}

% Figures and subfigures
\usepackage{graphicx}
\usepackage{subfigure}

% Improved tables
\usepackage{tabularx}
\usepackage{multirow}
\usepackage{longtable}
\usepackage{booktabs}
% To have appendices appear in the TOC
\usepackage[titletoc]{appendix}

% Needed for floating images
\usepackage{float}

% Improved verbatim appearance
\usepackage{fancyvrb}

% Math stuff
\usepackage{amsmath}
\numberwithin{equation}{section}
\usepackage{amssymb}
\usepackage{amsthm}

\newtheorem{theorem}{Theorem}

% Clickable links and references
\usepackage{hyperref}

% For code sections
\usepackage{minted}

\usepackage{changepage} % Serve per allargare le tabelle senza rompere i margini
\usepackage{afterpage}%pagine bianche finali

% biblatex is useful because it autodetects unused references
% and sorts used ones by occurrence.
\usepackage{csquotes}
\usepackage[style=apa , sorting=nty, uniquename=false]{biblatex}
\addbibresource{bib/Bibliography.bib}
\DefineBibliographyStrings{english}{
  bibliography = {References},}

% Only needed for placeholder text
\usepackage{lipsum}

% The content of the thesis is to be input in the appropriate .tex files
% referenced by the \input{} commands

%*******************************************************
% Pages' and front cover's settings
%*******************************************************
\input{settings/Settings}

%*******************************************************
% Cover
%*******************************************************
\input{front/Cover}

%*******************************************************
% Dedication
%*******************************************************
\input{front/Dedication}

% Roman page numbering for abstract, ToC and introduction
\frontmatter
\pagenumbering{Roman}

%*******************************************************
% Abstract
%*******************************************************
\input{front/Abstract}
\input{front/Sommario.tex}
%*******************************************************
% Table of contents
%*******************************************************
{
    \hypersetup{linkcolor=black}
    \tableofcontents
}

%*******************************************************
% Introduction
%*******************************************************
% Do not edit
\renewcommand{\chaptermark}[1]{\markboth{\MakeUppercase{\ #1}}{}}

\input{front/Introduction}

%*******************************************************
% Chapters
%*******************************************************
\mainmatter

% Do not edit
\renewcommand{\chaptermark}[1]{\markboth{\MakeUppercase{\chaptername\ \thechapter.\ #1}}{}}

% Add more \input{} commands as needed. Chapters can all be
% contained in one single .tex file, or you can have one file
% per chapter.

\input{main/Chapter1}
\input{main/Chapter2}
\input{main/Chapter3}
\input{main/Chapter4}
\input{main/Chapter5.tex}
\input{main/Chapter6.tex}


%*******************************************************
% Conclusion
%*******************************************************
\input{back/Conclusion}

%*******************************************************
% Appendix
%*******************************************************
% Do not edit
\pagestyle{fancy}
\renewcommand{\chaptermark}[1]{\markboth{\MakeUppercase{APPENDIX\ \thechapter.\ #1}}{}}

\appendix
\label{appendix}

\input{back/Appendix1.tex}


\backmatter

%*******************************************************
% Bibliography
%*******************************************************
\nocite{*}
\printbibliography[heading=bibintoc]
%*******************************************************
% FinalDedication
%*******************************************************
\input{back/FinalDedication}
\afterpage{\null\thispagestyle{empty}\newpage}
\afterpage{\null\thispagestyle{empty}\newpage}
\end{document}