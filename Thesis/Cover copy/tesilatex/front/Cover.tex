

%\usepackage[a4paper,top=4.5cm,bottom=4.5cm,left=2cm,right=2cm]{geometry}


% Do not edit
% Set up the transparent full-page background logo
\newcommand\BackgroundPicFront{
    \put(-4,0){
        \parbox[b][\paperheight]{\paperwidth}{
            \vfill
            \centering
            {\transparent{0.07}\includegraphics[width=\paperwidth,height=\paperheight,keepaspectratio]{img/units_logo.png}}
            \vfill
        }
    }
}

% Thesis's title and author
\title{Reclaiming the Narrative: Historical and Contemporary Role Models as Stimulus for Equity in Mathematics}
\author{Name Surname}
\begin{document}
\begin{titlepage}
    \newgeometry{a4paper,top=4.5cm,bottom=4.5cm,left=2cm,right=2cm}
    \begin{center}
    % Insert the background here
    %\AddToShipoutPicture{\BackgroundPicFront}
    {\LARGE {\bfseries \textbf{UNIVERSITY OF TRIESTE} \\}}
    \vspace{5mm}
    {\Large {\bfseries \textbf{Department of Mathematics, Informatics and Geosciences \\}}}
    \vspace{1cm}
    \includegraphics[width=6cm,height=6cm]{img/units_logo.png}\\[1cm]

    {\Large\textbf{
        Master's Degree in\\ Computational Mathematics and Modelling  \\
    }}
    \vspace{1cm}
    {\LARGE 
        {\bfseries \thetitle} %
    }
    \vspace{1cm}

    % Replace the date with the correct one before printing the final version!
    {\large \textbf{July 23, 2026} \\[1cm]
    }

    % Table for author's and (co-)supervisor's names
        {\large
            \setlength{\tabcolsep}{12pt}
            \begin{tabularx}{\linewidth}{ >{\raggedleft}X >{\raggedright\arraybackslash}X}
                \bfseries Candidate & \bfseries Supervisor \\
                \bfseries Giorgia Leoncilli & \bfseries Prof. Daniel Doz \\ [5mm] 
                
            \end{tabularx}
        }
    \textbf{A.Y. 2025/2026}
    \end{center}
\end{titlepage}
\ClearShipoutPicture
\restoregeometry{}